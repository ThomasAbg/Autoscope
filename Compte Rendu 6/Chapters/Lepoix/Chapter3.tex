...
Pour plus d'informations, le fonctionnement global de Yocto est décrit dans un précédent rapport de projet~: {\href{https://github.com/thomaslepoix/ESTEI/blob/master/M1_Projet_Telecommande_Raspberry-Pi/Hardware_Middleware.pdf}{\codeinline{text}{https://github.com/thomaslepoix/ESTEI/blob/master/M1_Projet_Teleco}\\\codeinline{text}{mmande_Raspberry-Pi/Hardware_Middleware.pdf}}}
...
\begin{itemize}[label=$\bullet$]
	\item Il existe une interface FTP destinée uniquement à récupérer les clichés du ciel stockés sur le télescope. L'utilisateur \codeinline{text}{autoscope} est le seul à pouvoir se connecter au serveur FTP et il ne sert qu'à ça.
	\item Il existe une interface SSH destinée uniquement à l'administration système du télescope. L'utilisateur \codeinline{text}{admin} est le seul à pouvoir se connecter au serveur SSH.
	\item L'utilisateur \codeinline{text}{root} n'est accessible que localement ou à distance via l'utilisateur \codeinline{text}{admin}.
	\item La console locale \codeinline{text}{tty1} est la console d'accueil du système. Elle est la seule à disposer d'un terminal.
	\item La console locale \codeinline{text}{tty2} affiche les messages générés par le système d'exploitation et notamment les messages du système d'initialisation.
	\item La console locale \codeinline{text}{tty3} affiche les messages générés par le daemon d'interface avec le GPS \codeinline{text}{mtk3339d}.
	\item La console locale \codeinline{text}{tty4} affiche les messages générés par le daemon \codeinline{text}{helloworld} s'il est présent.
	\end{itemize}
...
{\href{http://picamera.readthedocs.io/en/latest/recipes2.html#web-streaming}{\codeinline{text}{http://picamera.readthedocs.io/en/latest/recipes2.html#web-streaming}}}
...
{\href{https://picamera.readthedocs.io/en/latest/recipes2.html#rapid-capture-and-streaming}{\codeinline{text}{https://picamera.readthedocs.io/en/latest/recipes2.html#rapid-capture-and-stre}\\\codeinline{text}{aming}}}
...
Les messages produits par le daemon sont redirigés vers la console \codeinline{text}{/dev/tty4} accessible par la combinaison de touches \codeinline{text}{[CTRL]}, \codeinline{text}{[ALT]}, \codeinline{text}{[F4]}.
